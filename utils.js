...
const jwtProofParams = (jwt, credString='","sub":"') => {
    const [header, payload, signature] = jwt.split('.')
    const tbs = `${header}.${payload}`
    const payloadStartByte = header.length + 1
    const credPayloadStart = searchForPlainTextInBase64(credString, payload)[0]
    ...
    const remainder = credPayloadStart % 3
    const shiftedStart = credPayloadStart - remainder

    shiftedStartInB64 = shiftedStart * 4 / 3
    console.log(
        remainder, 
        payload, 
        btoa('","sub":"'), 
        btoa('creds":"'), 
        Buffer.from(payload.slice(shiftedStartInB64, shiftedStartInB64+12), 'base64').toString()
    )
    return {
        tbs: tbs
    }
...
jwtProofParams(twitterJwt, '{"creds":"')
